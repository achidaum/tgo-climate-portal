...
# 2. เชื่อมต่อ AI (สำหรับช่องแชทด้านล่างเท่านั้น)
...
# 5. ส่วนประเมินคาร์บอนรายวัน (กลับไปใช้ Link ตามข้อมูลเก่า)
st.subheader("🍃 บริการแนะนำ")

# ใช้หน้าตาแบบที่คุณเคยทำไว้ (ปุ่มใหญ่เน้นๆ)
with stylable_container(key="highlight", css_styles="button {background-color: #e8f5e9; border: 2px solid #2e7d32;}"):
    st.write("✨ **ระบบประเมินคาร์บอนรายวัน (Daily Carbon Footprint)**")
    # ลิงก์ไปยังแอปประเมินตัวเดิมของคุณ
    st.link_button("👉 คลิกเพื่อเข้าสู่ระบบประเมิน", "https://6ezbjfuuk36bisipg8y8bh.streamlit.app/", use_container_width=True)

st.write("---")

# 6. ส่วนปุ่ม GHG และ TGO (ข้อมูลเดิม)
...
# 7. ส่วน AI สำหรับตอบคำถาม (แยกส่วนชัดเจน)
...
if prompt := st.chat_input("พิมพ์คำถามที่นี่..."):
    ...
    with st.chat_message("assistant"):
        try:
            # AI จะทำหน้าที่แค่ตอบคำถามที่พิมพ์เท่านั้น
            response = model.generate_content(f"คุณคือผู้เชี่ยวชาญด้านก๊าซเรือนกระจก ตอบคำถาม: {prompt}")
